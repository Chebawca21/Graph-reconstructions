...
class GCNModelFeedback(GCNModelVAE):
    ...
    def __init__(self, placeholders, num_features, num_nodes, features_nonzero, nhidden1, nhidden2, nhidden3, vae, auto_scalar, **kwargs):
        self.nhidden3 = nhidden3
        self.autoregressive_scalar = auto_scalar
        super(GCNModelFeedback, self).__init__(placeholders, num_features, num_nodes, features_nonzero, nhidden1, nhidden2, vae, **kwargs)

    def make_decoder(self):
        self.l0 = GraphiteSparse(input_dim=self.input_dim,
                                      output_dim=self.nhidden3,
                                      act=tf.nn.relu,
                                      dropout=0.,
                                      logging=self.logging)

        self.l1 = Graphite(input_dim=self.nhidden2,
                                              output_dim=self.nhidden3,
                                              act=tf.nn.relu,
                                              dropout=0.,
                                              logging=self.logging)

        self.l2 = Graphite(input_dim=self.nhidden3,
                                              output_dim=self.nhidden2,
                                              act=lambda x: x,
                                              dropout=self.dropout,
                                              logging=self.logging)

        self.l3 = InnerProductDecoder(input_dim=self.nhidden2,
                                      act=lambda x: x,
                                      logging=self.logging)

        self.l4 = Scale(input_dim = self.nhidden2, logging = self.logging)
    ...
    def decoder(self, z):
        ...
        update = (1 - self.autoregressive_scalar) * z + self.autoregressive_scalar * update
    ...
    def sample(self):
        z = tf.random_normal([self.n_samples, self.nhidden2])
